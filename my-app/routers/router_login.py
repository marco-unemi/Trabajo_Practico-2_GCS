...
from controllers.funciones_home import count_empleados, count_usuarios
...
def inicio():
    if 'conectado' in session:
        return render_template(
            ...
            empleados_count=count_empleados(),
            usuarios_count=count_usuarios()
        )
    else:
    ...
    if 'conectado' in session:
        ...
        if request.method == 'POST' and 'email_user' in request.form and 'pass_user' in request.form:
            ...
            if conexion_MySQLdb is None:
                flash('No se pudo conectar con la base de datos. Verifica que el esquema `crud_python` exista.', 'error')
                return render_template(f'{PATH_URL_LOGIN}/base_login.html')
